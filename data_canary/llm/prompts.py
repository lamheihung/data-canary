...
1.  Clarity: Names should be descriptive and avoid abbreviations where a full word is clearer (e.g., use `identifier` instead of `id` unless contextually common).
2.  Case: Avoid PascalCase (e.g., `UserId`), camelCase (e.g. `userId`), and ALL_CAPS (e.g. `USER_ID`).
3.  Special Characters: Avoid spaces, hyphens, and other special characters.
4.  Format: All names must be in `snake_case` (e.g., `User_Id` --> `user_id`, `UserId` --> `user_id`, `uSER_Id` --> `user_id`).
